def name_node():
	...
	commands.getoutput("sshpass -p redhat ssh root@{} 'iptables -F'".format(ip_NN))
	...
	os.system("sshpass -p redhat ssh root@{} 'hadoop-daemon.sh start namenode'".format(ip_NN))
	os.system("sshpass -p redhat scp /root/project/namenode/VERSION root@{}:/mastern/current ".format(ip_NN))
	os.system("sshpass -p redhat scp /root/project/namenode/VERSION root@{}:/mastern/previous.checkpoint/ ".format(ip_NN))	
